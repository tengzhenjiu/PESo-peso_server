...
public class Stemmer implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 2743912270970998414L;
	...
	private String stepx(String str) {
		NewString stem = new NewString();

		if (str.charAt(str.length() - 1) == 's') {
			if ((hasSuffix(str, "shes", stem))
					|| (hasSuffix(str, "ses", stem))
					|| (hasSuffix(str, "ches", stem))
					|| (hasSuffix(str, "xes", stem))) {
				String tmp = "";
				...
				if (str.length() > 3
					...
					char xxx = str.charAt(str.length() - 4);
					if (xxx != 'a' && xxx != 'e'
							&& xxx != 'i'
							&& xxx != 'o'
							&& xxx != 'u') {
					...
					char xxx = str.charAt(str.length() - 2);
					if (xxx != 'i' && xxx != 's') {
						String tmp = "";
						for (int i = 0; i < str
								.length() - 1; i++)
	...
	private String step4(String str) {
		...
		String[] suffixes = { "al", "ance", "ence", "er", "ic", "able",
				"ible", "ant", "ement", "ment", "ent", "sion",
				"tion", "ou", "ism", "ate", "iti", "ous",
				"ive", "ize", "ise" };
	...
	private String stripPrefixes(String str) {
		...
		String[] prefixes = { "kilo", "micro", "milli", "intra",
	...
	public String stripAffixes(String str, boolean islightweight) {
		str = str.toLowerCase();
		str = Clean(str);
	...
	public String stemString(String str) {
		...
		for (int j = 0; j < str.length(); j++) {
			...
			if (Character.isLetterOrDigit(c)) {
				...
				if (start != -1) {
					result.append(stripAffixes(str
							.substring(start, j)));
					start = -1;
				}
				result.append(c);
		...
		if (start != -1) {
			result.append(stripAffixes(str.substring(start,
					str.length())));
		}
		return result.toString();
	}

	public static String getStemmer(String s1) {
		if (instance == null)
			instance = new Stemmer();
		return instance.stemString(s1);
	}

	public void test() {
		Stemmer stem = new Stemmer();

		String result = stem.stripAffixes("create");
		System.out.println(result);
		String result1 = stem.stemString("waving");
		System.out.println(result1);
		String str = "create several;keywords may is quite ; a good choice ; are your agree";
		System.out.println(str + "\n" + Stemmer.getStemmer(str));
	}
